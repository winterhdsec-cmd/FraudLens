...
3. **✔ 申报书当前版本 v17（16 页逐页质检全过）**（2026-08-31）：`docs/项目计划书_v17.docx` + PDF（页渲染 `docs/v17_pages/`，填充脚本 `check_pages_v17.py`）。项目名**《慧眼识诈——基于图神经网络的智能研判系统》**。配图演进：v15 深色科技风（gen_figures_v6）→ 用户反馈"打印不清晰/表格套表格/图题位置/图混乱空白多"→ v16 浅色学术风（gen_figures_v7，图题移到图下方、去 1x1 套表、每页填满）→ 用户再反馈"图没改好，用多模态能力重看"→ **v17 = v9 图（`gen_figures_v9.py`）+ 逐张视觉复检闭环**：切片（`slice_v9.py`，宽 1400 高 760）→ Read 逐片目检 → 存疑处 HTML 源码裁决 + PIL 像素分析。**本轮三修**：①Chrome headless 截图必须加 `--hide-scrollbars` 且视口高度大于内容（1300x3400）——否则滚动条/滑块伪影被 autocrop 当内容，fig1 曾被撑出 50% 假空白；②autocrop 行阈值加强为 `max(w*0.008,55)` 滤 30px 级滑块残影；③fig4 副标题"右图为同构示例"术语错误 → "左：团伙A · 右：团伙B · 案件6 为'孤证不并'反例"。v9 图要点：fig1 一页通（1714px 高，4 数字卡横排+五步+三创新+本质区别横幅）、fig2 四层架构底向上+层间数据流标注、fig3 垂直主干布局（①②并排→③→④质检→双出口，回环走左缘，无箭头穿卡）、fig4 单画布双团伙+案件6 反例+画像卡、fig5 双通道→∩→三条件→T 分支出牌、fig6 纯 SVG 柱状图（程序化坐标防标签重叠，诚实星标）、fig7 四象限+左四玩家卡+底三优势卡、fig8 三阶段+两长期红利。生成管线 `fill_template_v3.py`（幂等，FIG 指向 plan_figures_v9，SCALE 逐图微调）。剩余：用户内容纠错（人名/学校/联系方式）。
