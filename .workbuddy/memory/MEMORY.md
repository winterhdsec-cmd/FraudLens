...
- 交互层 Vue3+Element Plus+ECharts+vis-network；决策协同层 FastAPI + **LangGraph StateGraph 真反思闭环**（规划→预处理→分析→聚类→反思，条件边回连）；数据层 MySQL+Redis + BGE-large 本地推理 + DeepSeek 云端 + GNN。
...
- 缺口：TLS 待证书、真实警务数据端到端验证缺、案卷 OCR→结构化待接。
