...
- **新增 400 校验前先确认该 handler 有没有 `except HTTPException: raise`** —— 只有 `except Exception` 时，内部 `raise HTTPException(400)` 会被兜成 **500**。
- **终态不可逆**：所有状态机（冻结工单 / HITL 复核 / 审批流）的终态都不得再被流转或改写结论，否则留痕失效。修一处要顺手查同类。
- **写前端前先实拉接口 dump 真实字段名，再用断言锁住**（本项目多次栽在"前端读的字段后端不给"上；冻结执行器 `account` vs `account_number` 即此类）。见到 `ElMessageBox.alert(JSON.stringify(row))` 即视为"未完成的详情页"（已复发 4 次，现全部消灭）。
- **dump 字段必须深入到数组元素的形状**：只打印顶层键会漏掉嵌套结构。2026-09-20 我 dump 了审批流 `flow` 的顶层键却漏了 `nodes[0]`，结果新写的表决记录表「角色/结论/时间」三列全空（用了 `role`/`status`/`decided_at`，实际是 `approver_role`/`decision`/`created_at`）——**构建通过、断言全绿，只有看图才发现**。
- **grep 只能给候选，判定要看实际实现**：曾用"有 el-table 但无 v-loading"断定 5 个视图缺加载态，核实后发现 OverviewView 已有骨架屏（假阳性）。
...
- **构建通过 ≠ 界面能用**：办案工作台曾因 `computed` 漏写 `.value`（`appState.cases` 是 Ref 不是数组）在渲染期抛异常，只有带 `?case_id=` 进入才触发，被错误边界接住。**UI 改动必须真开页面看**，见 `frontend-visual-verify` 技能。
- **排查报错认第一个异常**：`el-tabs unregisterPane` 之类是**次生错误**，要往回追根因；用「回退到上一提交再跑一次」的对照实验确认归属。
- **取样陷阱**：用 `.query().first()` 取样做验证会赌运气（曾取到唯一那条 `is_active=False` 记录，误判碰撞功能有 bug）。
