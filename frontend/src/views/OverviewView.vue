...
<div class="view-section">
  <SectionHeader icon="DataAnalysis" title="案件总览" desc="查看所有录入案件信息，点击可进入案件详情页">
    <div class="view-toggle">
      <el-button-group>
        <el-button size="small" :type="viewMode === 'card' ? 'primary' : ''" @click="viewMode = 'card'"><el-icon><Grid /></el-icon> 卡片</el-button>
        <el-button size="small" :type="viewMode === 'table' ? 'primary' : ''" @click="viewMode = 'table'"><el-icon><Tickets /></el-icon> 列表</el-button>
      </el-button-group>
    </div>
  </SectionHeader>

  <AnalysisAdvisory
    :abnormal="analysisAbnormal"
  ...
  <div class="stats-overview">
    <div class="stat-card tech-card">
      <div class="stat-icon-wrapper danger"><el-icon class="stat-icon"><Files /></el-icon></div>
      <div class="stat-content"><div class="stat-value">{{ cases.length }}</div><div class="stat-label">案件总数</div></div>
    </div>
    <div class="stat-card tech-card">
      <div class="stat-icon-wrapper warning"><el-icon class="stat-icon"><User /></el-icon></div>
      <div class="stat-content"><div class="stat-value">{{ gangs.length }}</div><div class="stat-label">涉案团伙</div></div>
    </div>
    <div class="stat-card tech-card">
      <div class="stat-icon-wrapper success"><el-icon class="stat-icon"><Money /></el-icon></div>
      <div class="stat-content"><div class="stat-value">{{ totalAmountFormatted }}</div><div class="stat-label">涉案金额</div></div>
    </div>
    <div class="stat-card tech-card">
      <div class="stat-icon-wrapper info"><el-icon class="stat-icon"><Aim /></el-icon></div>
      <div class="stat-content"><div class="stat-value">{{ successRate != null ? successRate + '%' : '—' }}</div><div class="stat-label">研判准确率</div></div>
    </div>
  </div>
    ...
    <div v-if="viewMode === 'card'" class="cases-card-grid">
      <div v-for="c in pagedCases" :key="c.id" class="case-card tech-card" @click="viewCaseDetail(c)">
        ...
        <div class="case-card-meta">
          <span><el-icon><User /></el-icon> {{ c.victimName || '未知' }}</span>
          <span><el-icon><Calendar /></el-icon> {{ c.date || c.created_at ? (c.date||c.created_at).slice(0,10) : '-' }}</span>
        </div>
        <div class="case-card-footer">
          <el-tag :type="c.status === '已立案' ? 'warning' : c.status === '侦办中' ? 'primary' : 'success'" size="small">{{ c.status }}</el-tag>
          <span class="case-card-gang" v-if="getCaseGang(c.id)"><el-icon><User /></el-icon> {{ getCaseGang(c.id).name }}</span>
        </div>
      </div>
    </div>
  ...
  <div v-if="gangs.length" class="gangs-section">
    <div class="gangs-card">
      <div class="section-sub-header">
        <h3 class="sub-title"><el-icon class="sub-icon"><User /></el-icon>关联团伙</h3>
...
import AnalysisAdvisory from '../components/AnalysisAdvisory.vue'
...
const {
  ...
  reloadCasesAndGangs, loadDashboard, initCharts,
